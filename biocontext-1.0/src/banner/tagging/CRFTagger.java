...
*/
        ...
        //TODO: changed
		Pipe pipe = forwardCRF.getInputPipe();
		InstanceList il = new InstanceList(pipe);
		
        Instance instance = new Instance(sentence.getTrainingText(format, reverse), sentence.getTag(), null, null, pipe);
        //il.addThruPipe(instance);
        
        il.add(instance);
        
        if (il.size() != 1)
        	throw new IllegalStateException("Expected only one instance from the pipe!");
        
        Sequence input = (Sequence) ((Instance)il.get(0)).getData();
        
        Sequence tags = forwardCRF.viterbiPath(input).output();
        
        //Sequence tags = new MaxLatticeDefault(forwardCRF,input).bestOutputSequence();

        
        //TODO: changed
        /*if (positions.length != tags.size())
            throw new IllegalArgumentException("p: " + positions.length + ", t: " + tags.size());
        if (types.length != tags.size())
            throw new IllegalArgumentException("ty: " + types.length + ", ta: " + tags.size());*/
        
        if (types.length != tags.size() || positions.length != tags.size()){
        	System.err.println("CRFTagger.getPositionsAndType: error");
