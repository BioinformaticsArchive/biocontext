...
public class Medline implements DocumentIterator {
	...
	public uk.ac.man.documentparser.dataholders.Document next() {
		...
		MyNodeList absNodes = XPath.getNodeList("Article/Abstract/AbstractText", n);
		String abs = null;
		if (absNodes != null && absNodes.getLength() > 0){
			abs = "";
			for (Node absNode : absNodes){
				if (absNode.getAttributes().getNamedItem("Label") != null){
					if (abs.length() > 0)
						abs += "\n";
					abs += absNode.getAttributes().getNamedItem("Label").getTextContent() + "\n";
				}
				
				abs += absNode.getTextContent();				
			}
		}
		...
		if (abs == null){
			Node absNode = XPath.getNode("OtherAbstract/AbstractText", n);
