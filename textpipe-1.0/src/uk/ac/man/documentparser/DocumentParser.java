...
import uk.ac.man.documentparser.input.EBI;
...
import uk.ac.man.documentparser.input.NCBIEUtilsSearcher;
...
public class DocumentParser {
	...
	public static DocumentIterator getDocuments(ArgParser ap, Logger logger){
		...
		if (ap.containsKey("pmcAbs")){
			...
			documents = new Directory(ap.getFiles("textDir"),new TextFileFactory(),".txt",ap.containsKey("recursive"));
		...
		} else if (ap.containsKey("EBI")){
			File f = ap.getFile("EBI");
			documents = new EBI(f);
		} else if (ap.containsKey("eutils")){
			documents = new NCBIEUtilsSearcher(ap.get("eutils"));
		}
